...
class Solution():
        ...
        for i in range(len(x0_all)):
            ...
            if self.con.apply(x0.tolist()):
                ...
                try: 
                    ...
                    if res.status != 0 or any(x_feasible < 0.0) or any(x_feasible > 1.0):
                        continue

                except:
                    continue
            
            # if self.con.apply(x_feasible):
            #     print "True"
    ...
    def optimize_feasible(self, x0):
        res = optimize.minimize(self.f, x0, method='SLSQP', 
                       constraints=[self.ineq_cons], options={'ftol': 1e-8, 'disp': True},
                       bounds=self.bounds)
        # print 'Optimal: ', res.x
        return res
